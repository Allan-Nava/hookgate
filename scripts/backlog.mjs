...
// stable HG-n id and a trailing `<!-- hg: ... -->` metadata comment. ROADMAP.md and
...
const REPO_BLOB = 'https://github.com/Allan-Nava/hookgate/blob/main'
const DASH = ' — '
...
export function parse(text) {
  ...
  const flush = () => {
    ...
    const m = raw.match(/^- \[( |x)\] \*\*(HG-(\d+))\s+—\s+(.+?)\*\*:?\s*([\s\S]*)$/)
...
export function lint(model) {
  ...
  for (const it of model.items) {
    ...
    if (!it.meta) {
      err(it.line, `${it.id} has no <!-- hg: ... --> metadata`)
      continue
    }
...
export function roadmap(model) {
  ...
  const out = []
  out.push('# Roadmap — hookgate', '', '<!-- GENERATED by scripts/backlog.mjs roadmap — do not edit by hand. -->', '')
...
\`BACKLOG.md\` is the single source of truth: it carries the stable \`HG-n\` id that commits and the CHANGELOG reference, and [ROADMAP.md](${REPO_BLOB}/ROADMAP.md) is generated from it. This issue is a view of that item, kept in step by \`scripts/backlog.mjs issues --apply\`, so closing it means ticking the item in the backlog and regenerating the roadmap in the same commit.
`
}
