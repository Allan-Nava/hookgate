...
const REPO = 'https://github.com/Allan-Nava/hookgate'
const BLOB = `${REPO}/blob/main`
const SITE = 'https://allan-nava.github.io/hookgate/'
...
// status note. No pipeline diagram: this plugin has two gates, not six phases.
  ...
  return cards
    .map(
      (c) => `      <article class="card">
        ...
        <p>${esc(c.status.replace(/^hookgate:\s*/, ''))}</p>
        <ul class="refs">
          <li><a href="${BLOB}/bin/hookgate.mjs"><code>hookgate ${esc(c.handler)}</code></a></li>
...
const headline = `${title} — calibrated gates for Claude Code hooks`
...
<meta name="theme-color" content="#2f5d8a" media="(prefers-color-scheme: light)">
...
:root {
  ...
  --accent: #2f5d8a; --accent-soft: #e6eef7; --code-bg: #f4f1ec;
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --accent: #7fa7d6; --accent-soft: #16212d; --code-bg: #1c1a17;
  }
}
...
<footer>
  <div class="wrap row">
    <span>MIT · <a href="${REPO}">Allan-Nava/hookgate</a></span>
...
// what gets hookgate listed there; a robots.txt at /hookgate/ would be dead weight.
